...
export species="Abyssorchomene_rb"
...
cat 05_full_out/${species}.full_mask.out | tail -n +4 | awk -v OFS="\t" '{ print $6, $7, $11 }' | 
	awk -F '[\t/]' -v OFS="\t" '{ if (NF == 3) print $3, "NA", $2 - $1 +1; else print $3, $4, $2 - $1 +1 }' |  
	datamash -sg 1,2 sum 3 | grep -v "\?" | 
	awk -v OFS="\t" -v genomeLen="${allLen}" '{ print $0, ($3 / genomeLen)*100 }' > 05_full_out/${species}.full_mask.tabulate
